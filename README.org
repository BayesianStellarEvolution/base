...
#+HEADER: :noweb-ref main sequence spec
#+BEGIN_SRC haskell
spec :: SpecWith ()
spec = parallel $ do
...
#+HEADER: :noweb-ref main sequence
#+BEGIN_SRC haskell
...
#+HEADER: :noweb-ref interpolate spec
#+BEGIN_SRC haskell
spec :: SpecWith ()
spec = do
  logInterpolateSpec
...
( TODO ) Write tests to verify that invalid model files cause reasonable failure here
( TODO ) Should we also be calling ~convertModel~ for the conversion confirmation?
...
#+HEADER: :noweb-ref makeIsochrone
#+BEGIN_SRC haskell
